...
def generate_ix_rel():
    ...
    # 数据持久化
    save_path = "../000LocalData/IXVis/ix_list.csv"
    write_to_csv(ix_list, save_path)
    save_path = "../000LocalData/IXVis/as_list.csv"
    ...
    print("AS记录数:", len(as_list))
    print("IX-AS关系数量（数据处理后）:", len(ix_as_rel))

    return ix_list, as_list, ix_as_rel
...
def generate_draw_json():
    ...
    :return:
    """
    ix_list, as_list, ix_as_rel = generate_ix_rel()
    type_group = ["AS", "IX"]
    ...
    for item in ix_as_rel:
        temp_dict["source"] = "ix-id" + str(item[0])
        temp_dict["target"] = "as-id" + str(item[1])
...
if __name__ == "__main__":
    ...
    draw_rel(opt_title_name).render("../000LocalData/IXVis/global_ix.html")
