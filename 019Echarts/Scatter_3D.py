# coding:utf-8
"""
create on Jan 6,2020 By Wayne Yu

测试pyecharts的3D散点图

"""

import random

from pyecharts.faker import Faker
from pyecharts import options as opts
from pyecharts.charts import Scatter3D
from pyecharts.globals import ThemeType


def scatter3d_base() -> Scatter3D:
    data = [
        [random.randint(0, 100), random.randint(0, 200), random.randint(0, 100)]
        for _ in range(10000)
    ]
    print(data)
    c = (
        Scatter3D(init_opts=opts.InitOpts(width="1920px", height="900px", page_title="全球互联网网络3D散点图", theme=ThemeType.DARK))
        .add("随机散点",
             data,
             grid3d_opts=opts.Grid3DOpts(width=300, height=200, depth=300, rotate_speed=5, is_rotate=True, rotate_sensitivity=2),
             # xaxis3d_opts=opts.Axis3DOpts(type_=""),
             # yaxis3d_opts=opts.Axis3DOpts(type_=""),
             # zaxis3d_opts=opts.Axis3DOpts(type_="")

             )
        .set_global_opts(
            title_opts=opts.TitleOpts("全球互联网网络3D散点图"),
            visualmap_opts=opts.VisualMapOpts(range_color=Faker.visual_color),
        )
    )
    print(Faker.visual_color)
    return c


scatter3d_base().render("scatter_render.html")
