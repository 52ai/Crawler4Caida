...
class ProjectNetwork extends React.Component {
  constructor() {
    super();
  ...
  componentDidMount() {
    ...
    let graphData = generateGraphData(this.props.currentNode, this.props.data);
  ...
  render() {
    ...
    return (
      <div>
        <canvas width={320} height={360} id="project-network" />
      </div>
    );
  }
}

const generateGraphData = (currentNode, data) => {
  const generateNodes = (nodes) => {
    ...
    return nodes.map((n) => {
      return {
        ...
        itemStyle: {
          color:
            currentNode && currentNode === n.name
              ? FOCUSED_NODE_COLOR
              : getColorMap(n.value),
        },
      };
