...
V7:
统计4134、4809、4837、9929、9808五张网，分别的出口互联关系（分为一次互联和二次互联）
...
def external_as_analysis(country, as2country, as_list, org_name):
    ...
    for path_item in file_path[-5:-4]:
        print(path_item)
        # 遍历一次文件，获取该国出口AS的数量
...
if __name__ == "__main__":
    ...
    as_list = ['4134', '4809', '4837', '9929', '9808']  # 存储需要统计的AS列表
    for item_as in as_list:
        item_as_direct_name = str(item_as) + "_direct"
        external_country_as_direct = external_as_analysis(country,
                                                          as2country_dict,
                                                          [item_as],
                                                          item_as_direct_name)

        item_as_indirect_name = str(item_as) + "_indirect"
        external_country_as_direct.extend([item_as])
        external_country_as_indirect = external_as_analysis(country,
                                                            as2country_dict,
                                                            external_country_as_direct,
                                                            item_as_indirect_name)
