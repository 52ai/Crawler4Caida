# coding:utf-8
"""
create on Jan 19, 2020 By Wayne YU

Function:

俄罗斯断网事件分析

俄罗斯TOP100，AS号互联关系变化（总体、国内、国际）（最近2年）
...
def gain_top_as_ru(as_info):
    """
    根据传入的最新的as info 信息获取ru top as 信息（TOP 100 & AS与国家对应的字典）
    :param as_info:
    ...
    ru_as_info.sort(reverse=True, key=lambda elem: int(elem[1]))
    print("RU Active AS Num:", len(ru_as_info))
    as_top_100_ru = []
    for as_item in ru_as_info[0:100]:
        as_top_100_ru.append(as_item)
    # save path
    save_path = "..\\000LocalData\\RUNet\\as_top_100_ru.csv"
