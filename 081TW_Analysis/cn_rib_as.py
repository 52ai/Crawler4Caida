...
def rib_analysis(rib_file):
    ...
    direct_route_as_dict = {}  # 按照自治域网络维度存储我直联路由依赖情况
    for line in file_read.readlines():
        line = line.strip().split("|")
        # if line[4] != "4134":
        #     continue
        ...
        if path_index < len(as_path):
            ...
            if direct_route_as not in direct_route_as_dict.keys():
                direct_route_as_dict[direct_route_as] = len(IP(v4_prefix))
            else:
                direct_route_as_dict[direct_route_as] += len(IP(v4_prefix))
    ...
    """
    按自治域网络维度，构建我国际路由获取依赖情况
    """
    direct_route_as_list = []  # 将自治域网络维度路由依赖转为列表
    direct_route_as_v4_num = 0  # 自治域网络维度的IP地址总量
    for item in direct_route_as_dict.keys():
        direct_route_as_list.append([item,
                                     direct_route_as_dict[item]])
        direct_route_as_v4_num += direct_route_as_dict[item]

    print("我国际直联自治域网络数量（RIB探测)：", len(direct_route_as_list))
    print("自治域网络维度的IP地址总量:", direct_route_as_v4_num)

    direct_route_as_list.sort(reverse=True, key=lambda elem: int(elem[1]))
    direct_route_as_result = []  # 存储结果
    for item in direct_route_as_list:
        # print(item)
        item_as = item[0]
        item_v4_num = item[1]
        item_as_country = "ZZ"
        ...
        try:
            item_as_org = as2org[item_as].strip("\"")
        except Exception as e:
            except_info_list.append(e)
        temp_line = [item_as, item_as_org, item_as_country, item_v4_num, round(item_v4_num/direct_route_as_v4_num, 4)]
        print(temp_line)
        direct_route_as_result.append(temp_line)
    save_file = "direct_route_as_result.csv"
    write_to_csv(direct_route_as_result, save_file)
