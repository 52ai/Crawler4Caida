...
根据as_core_map_data_integrate数据，生成极坐标并绘图，着重表示中国的AS连接数据
...
def draw_polar_map(as_info, open_file, year_str):
    ...
    for item in as_info:
        # print(item)
        if float(item[9]) > max_radius:
            max_radius = float(item[9])
        if float(item[9]) < min_radius:
            min_radius = float(item[9])
            min_index = item_cnt
        angle = (float(item[8]) / 360.0) * 2 * np.pi
        radius = float(float(item[9]))
    ...
    for item in radius_list:
        if item < max_radius * 0.2:
            ...
            if as_info[index_cnt][5] == "CN":
                cn_index.append(index_cnt)
        elif item < max_radius * 0.4:
            area_list.append(1)
            lw_list.append(0.1)
            c_color_list.append([float(256.0/256), float(256.0/256), float(256.0/256)])
            z_order_list.append(2)
            if as_info[index_cnt][5] == "CN":
                cn_index.append(index_cnt)
        ...
        # 如果该点为中国，则改变其填充颜色,改变其Marker
        if as_info[index_cnt][5] == "CN":
            cn_all_as.append(as_info[index_cnt])
            del c_color_list[-1]
            c_color_list.append([float(100.0/256), float(100.0/256), float(200.0/256)])
        index_cnt += 1
    print("CN all AS Length:", len(cn_all_as))
    cn_all_as.sort(reverse=True, key=lambda elem:int(elem[1]))
    cn_as_list = []  # 存储左右cn AS号
    for item_as in cn_all_as:
        cn_as_list.append(item_as[0])
    # print("- - - - - - - - - - - - - - - - - - - - - - - - ")
    ...
    for line in file_read.readlines():
        ...
        if line[0] in cn_as_list and line[1] in cn_as_list:
            ...
            if p1 and p2:
                alpha_value = 1
                line_width = 0.02
                line_color = [float(100/256), float(100/256), float(200/256)]
                z_order_value = 1
        ...
        elif line[0] in cn_as_list or line[1] in cn_as_list:
            p1 = coordinate_dic.get(line[0])
            p2 = coordinate_dic.get(line[1])
            if p1 and p2:
                alpha_value = 1
                line_width = 0.05
                line_color = [float(200 / 256), float(100 / 256), float(100 / 256)]
                z_order_value = 3
                print("computing:", p1, p2)
                ax.plot([p1[0], p2[0]], [p1[1], p2[1]], linewidth=line_width, alpha=alpha_value, color=line_color, zorder=z_order_value, )

    # ######################## 打点######################################
    c = ax.scatter(angle_list, radius_list, c=c_color_list, edgecolors=[0, 0, 0], marker="s", lw=lw_list, s=area, cmap='hsv', alpha=0.8, zorder=7)
    ...
    # plt.xticks([])  # 去掉横坐标
    # plt.yticks([])  # 去掉纵坐标
    save_fig_name = "../000LocalData/as_compare/as_core_map_integrate_cn" + year_str + ".jpg"
...
if __name__ == "__main__":
    ...
    year_string = "2019"
    file_in = '..\\000LocalData\\as_compare\\as_core_map_data_integrate20191203.csv'
    ...
    bgp_file = "..\\000LocalData\\as_compare\\as_rel_20191203_integrate.txt"
