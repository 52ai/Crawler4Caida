...
新版本ISP：将foreatlas2算法应用于全球TOP ISP互联关系拓扑结构可视化中，仅需构建输入图即可（节点和边）
...
def draw_2d(G_2d, pos, graph_name, is_draw=True, is_show=False):
    ...
    if is_draw:
        ...
        save_path = "../000LocalData/OPNet/" + graph_name + ".png"
...
def my_layout_ani():
    ...
    # line_ani.save('../000LocalData/OPNet/new_draw_2d_layout_ani_isp.gif', writer="imagemagick", savefig_kwargs={'facecolor': 'black'}, dpi=100)
    line_ani.save('../000LocalData/OPNet/new_draw_2d_layout_ani_isp.mp4', writer="ffmpeg",  savefig_kwargs={'facecolor': 'black'}, dpi=360)
    # plt.show()
...
if __name__ == "__main__":
    # 生成300个点，距离阈值为0,3的图
    # G = networkx.random_geometric_graph(10, 0.17, dim=2)
    # 读取新构建的图
    G = new_as_graph()
