...
第一步，统计所有登陆点的连通度
第二步，根据连通度计算极径，并生成每个点的极坐标
第三步，根据点的坐标和互联关系，绘制极图

直接就第三阶段了，新建海缆全部断；C-U的缆，各段关系都段。
...
def gain_format_data():
    ...
    需要统计每个登陆点的国别
    """
    landing2country_dict = dict()  # 存储每个登陆站的国别信息
    ...
    for row in cable_info_sheet.rows:
        ...
        if landing not in landing2country_dict.keys():
            landing2country_dict[landing] = country
    print(len(landing2country_dict.keys()))

    """
    需要统计每个点的连通度
    """
    cable_1 = ["southeast-asia-japan-cable-2-sjc2", "hainan-hong-kong-submarine-cable-system"]
    landing_degree_dict = dict()  # 存储每个登陆站的连通度
    rows_cnt = 1  # 行计数，初始化为1
    for row in cable_rel_sheet.rows:
        ...
        if landing2country_dict[row_list[1]] == "China" and landing2country_dict[row_list[2]] == "United States":
            pass
        if landing2country_dict[row_list[1]] == "United States" and landing2country_dict[row_list[2]] == "China":
            pass

        if row_list[1] not in landing_degree_dict.keys():
            landing_degree_dict[row_list[1]] = 1
        if row_list[1] in landing_degree_dict.keys():
            landing_degree_dict[row_list[1]] += 1

        if row_list[2] not in landing_degree_dict.keys():
            landing_degree_dict[row_list[2]] = 1
        if row_list[2] in landing_degree_dict.keys():
            landing_degree_dict[row_list[2]] += 1

    print(len(landing_degree_dict.keys()))
    """"
    统计登陆点信息
    """
    ...
    for row in cable_info_sheet.rows:
        ...
        for cell in row:
            ...
            row_list.append(cell.value)
        # print(row_list)
        arg_value = landing_degree_dict[row_list[4]]  # 将节点的值设置为其连通度
        angle = 0.0
        ...
        if row_list[4] not in landing_point_dict.keys():
            landing_point_dict[row_list[4]] = [arg_value, angle, country, arg_value]
    ...
    print("登陆点中连通度最大值为:%s" % max_arg_value)
    print("登陆点中连通度最小值为:%s" % min_arg_value)
...
def draw_polar_map():
    ...
    全球TOP点
    """
    save_path = "../000LocalData/CableMap/TopLandingPoint20Global_3(degree).csv"
    ...
    save_path = "../000LocalData/CableMap/TopLandingPoint20China_3(degree).csv"
    ...
    save_path = "../000LocalData/CableMap/TopLandingPoint20US_3(degree).csv"
    ...
    plt.axis('off')
    save_fig_name = "../000LocalData/CableMap/as_core_map_scatter_cable_3(degree).jpg"
