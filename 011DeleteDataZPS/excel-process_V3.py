...
def delete_specific_rows(filename,provence_list,carrier_list,delete_days_list):
    ...
    # 注意 转换时间时，不能用DataFrame，否则会出错
    print(pd.to_datetime(data['测试时间'], format="%Y-%d-%y", errors='coerce').dt.floor('d').isin(delete_days_list))

    excel_data = pd.DataFrame(data)

    print(excel_data[pd.to_datetime(data['测试时间'], format="%Y-%d-%y", errors='coerce').dt.floor('d').isin(delete_days_list)].index)
    ...
    for list_index in range(0,6):
        ...
        excel_data = excel_data.drop(excel_data[(excel_data['源省份'] == provence_list[list_index]) \
                                                ...
                                                & (pd.to_datetime(data['测试时间'], format="%Y-%d-%y", errors='coerce').dt.floor('d').isin(delete_days_list))].index)

        #删除符合条件的目的点
        excel_data = excel_data.drop(excel_data[(excel_data['目的省份'] == provence_list[list_index]) \
                                                & (excel_data['目的运营商'] == carrier_list[list_index]) \
                                                & (pd.to_datetime(data['测试时间'], format="%Y-%d-%y", errors='coerce').dt.floor('d').isin(delete_days_list))].index)
...
def main():
    ...
    #filename = "IPv6_TCP_20190708_20190709.xlsx"
    filename = "test.xlsx"
