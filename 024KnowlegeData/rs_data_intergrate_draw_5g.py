...
def data_intergrate_draw(res_auto_file_path, res_manual_file_path, des_file_path):
    ...
    for row in worksheet.rows:
        ...
        # print(key_words_str)
        if key_words_str.find("5G") != -1:
            # print(key_words_str)
            str_5g = str_5g + key_words_str
    ...
    # print(str_5g)
    str_5g_list = str_5g.split("、")
    print(str_5g_list[0:-1])
    str_5g_dict = {}
    for item in str_5g_list:
        if item not in str_5g_dict.keys():
            str_5g_dict[item] = 1
        else:
            str_5g_dict[item] += 1
    # print(str_5g_dict)
    str_5g_result = []  # 存储最终数据
    for key in str_5g_dict.keys():
        str_5g_result.append([key, str_5g_dict[key]])
    str_5g_result.sort(reverse=True, key=lambda elem: elem[1])
    print(str_5g_result)
    # print(res_manual_list)
    save_path = "5g_cloud.csv"
    write_to_csv(str_5g_result, save_path)
    """
